...
const otpVerify = async() => {
    ...
    const userid= localStorage.getItem("userid_n");
    console.log(userid)
    if(!userid){
    ...
    // const userid = JSON.parse(userDataID);
     console.log(userid);
    const userData=await fetch(`https://chaam-notes-app.onrender.com/api/auth/get-real-user-data/${userid}`,{
                method:"GET",
                headers:{
    ...
    //if(userTime.success){
        const createdAt=userTime.otpCreatedAt;
    ...
    auth_submit.addEventListener("click",async()=>{
        ...
        const response = await fetch("https://chaam-notes-app.onrender.com/api/auth/password-change-otp",{
                                method:"PUT",
                            ...
                            .then((dataotp)=>{
                                if(dataotp.success){
                                    ...
                                    localStorage.setItem("userid", dataotp.userid);
    ...
    for(let i = 0; i < 4; i++){
            ...
            if(auth_otp_b.value){
                ...
                if(i === 3){
                    ...
                    const response = await fetch(
                        "https://chaam-notes-app.onrender.com/api/auth/check-password-change-otp",
                        {
                            method:"POST",
                    ...
                    if(data.success){
                        localStorage.removeItem("userid_n");
                        localStorage.setItem("useridotp", data.userid);
                        window.location.href =
                        "../public/login_pwd_change.html";
        ...
        auth_otp_b.addEventListener("keydown",(e)=>{
            if(e.key === "Backspace" && auth_otp_b.value === ""){
